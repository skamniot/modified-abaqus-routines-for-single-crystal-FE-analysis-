...
c
      ...
      real*8, intent(in) :: dtime, signtau(nSys),signtautwin(nTwin)
      ...
      real*8, intent(in) :: xTwinNorm(nTwin,3),xTwinDir(nTwin,3)
      real*8, intent(in) :: tautwin(nTwin),tauctwin(nTwin)
      ! twin volume fraction
      real*8, intent(in) :: twinvolfrac(nTwin), twinvolfractotal 
c
...
c          
              ...
              result4=result4+(gamma0dot*nsliprate/tauc(I))*
     +                dtime*result1*SNNS
c
...
c
          ...
          if ((xtau >= 0.5 .or. twinvolfrac(I) > 0.5) .and. 
     +         twinvolfrac(I) < 1.0) THEN
c
              gammaTwinDot(I)=gamma0dottwin*(xtau**nsliprate)*
     +                     signtautwin(I) ! signtautwin always positive
c
...
c
              ...
              if (twinvolfrac(I) > 0.5) then
                gammaTwinDot(I)=gammaTwinDot(I)+0.299*
     +                           (1.0-twinvolfrac(I))
              end if
c
...
c
              ! result4 = dt dL_{ij}/dsigma_{kl} with indices in Voigt notation
              ! such that 4,5,6 components contributes as (b_i n_j + b_j n_i)
              result4 = result4 + (gamma0dottwin*nsliprate/tauctwin(I))*
     +                  dtime*result1*SNNS
c
              ! update plastic velocity gradient
