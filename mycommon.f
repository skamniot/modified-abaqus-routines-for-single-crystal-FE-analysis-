...
!Use this to avoid editing multiple common blocks independently
		integer, parameter :: nElements = 60000
		integer, parameter :: nintpts = 8
		integer, parameter :: nsdv  = 156
...
c
     ...
     +    kNoNeighbours(nElements,nintpts,2), 
          ! number of up/down neighbours
     +    kGrainIndex(nElements,nintpts),
     +    kSigma0(nElements,nintpts), ! output the max stress to failure
     +    kDeltaEff(nElements,nintpts), 
          ! output the max effective opening
     +    NUpDownExceeded 
          ! keep track of the max value of the neighbouring IP reached
