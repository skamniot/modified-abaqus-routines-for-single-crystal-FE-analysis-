...
c
      sigma0 = sigma0 - 400.0*max(abs(tvfneigh1(1)-tvfneigh2(1)),
     1      abs(tvfneigh1(2)-tvfneigh2(2)))
c
...
c 
      ...
      ! nrhs = 1 (apart from Riks analysis)
...
c
      smaxratio = 0.1
      if (popn .GT. 0.0) then
        deltaortiz = sqrt(smaxratio*popt*popt + popn*popn) 
      ! effective displacement > 0
      else
      ...
      if (damage .LT. 1.0) then
        if (deltaortiz .GT. deltaF) then ! fully damaged
          ...
          if (deltaortiz .GT. deltaD) then ! damage increase
            damage = deltaF*(deltaortiz-delta0)/
     &               (deltaortiz*(deltaF-delta0))
